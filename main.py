...
async def club(callback: types.CallbackQuery):
  await bot.delete_message(callback.from_user.id, callback.message.message_id)
  await bot.send_message(callback.from_user.id, "Ты начал заполнение формы в клуб,\nВведи своe ФИО")
...
async def state1(message: types.Message, state: FSMContext):
  ...
  await message.answer("Введи свой год окончания")
...
async def state1(message: types.Message, state: FSMContext):
  ...
  await message.answer("Введи свой факультет-направление")
